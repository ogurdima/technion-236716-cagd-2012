...
{
	return &list[CtlPlygn].where[ptIndex];
}

CCagdPoint *cagdGetPoint(UINT Point)
{
	return &list[Point].where[0];
}*/


CCagdPoint cagdMultiply(const CCagdPoint* pt, double scalar)
{
	CCagdPoint newPoint;
	newPoint.x = pt->x * scalar;
	newPoint.y = pt->y * scalar;
	newPoint.z = pt->z * scalar;
	return newPoint;
}

CCagdPoint cagdAdd(const CCagdPoint* ptA, const CCagdPoint* ptB)
{
	CCagdPoint newPoint;
	newPoint.x = ptA->x + ptB->x;
	newPoint.y = ptA->y + ptB->y;
	newPoint.z = ptA->z + ptB->z;
	return newPoint;
}