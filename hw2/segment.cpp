...
CCagdPoint *cagdGetVertex(UINT id, UINT vertex)
{
  SEGMENT *segment;
  if(!valid(id))
    return NULL;
  ...
  if(segment->length <= vertex)
    return NULL;
//  memcpy(where, &segment->where[vertex], sizeof(CCagdPoint)); // who uses memcpy???
  return &segment->where[vertex];
}
...
bool cagdSetVertex(UINT id, UINT vertex, const CCagdPoint *where)
{
  ...
  if(segment->length <= vertex)
    return FALSE;
//  memcpy(&segment->where[vertex], where, sizeof(CCagdPoint)); // make sure not to overwrite all data, just location
